...
vector<int> choose_nbor( vector<int> ourchoice, vector<int> other) {
	int stock;
	int orsz=ourchoice.size();
	#pragma omp parallel
	{
	#pragma omp for
	...
	#pragma omp critical
	{
			remove(other.begin(), other.end(), ourchoice[i]);
			other.pop_back();
	}
		}
  }
...
pair<double, matrix> HM(matrix& cov,matrix& mean, set<string> &stocklist, set<string>& must, const int& num, const double& miu){
	...
	for(int i=0;i<trytime;++i){
		vector<int>newourchoice=choose_nbor(ourchoice, other);
